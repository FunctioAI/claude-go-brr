...
[[ "$(<"$TMP/exchange.out")" == *'"installation_required":true'* ]] || fail "host repository decision was not displayed"
...
[[ "$(<"$TMP/calls")" == *'github install-url --remote origin'* ]] || fail "host was not asked to decide public repository access"
[[ "$(<"$TMP/public.out")" == *'"installation_required":false'* ]] || fail "host public repository decision was not displayed"
